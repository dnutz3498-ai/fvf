...
class NetManager {
  ...
  // ── ICE server config — STUN + free TURN servers so NAT traversal works
  // on GitHub Pages (HTTPS) and across different network types.
  // Free TURN from Open Relay Project (metered.ca) — no key needed.
  _iceServers() {
    return [
      ...
      // Open TURN relays — essential when both peers are behind symmetric NAT
  ...
  // ── PeerJS options shared by host and guest
  _peerOpts() {
    return {
      // Explicitly use the PeerJS cloud v1.5 endpoint
      host: '0.peerjs.com',
      port: 443,
      path: '/',
      secure: true,          // must be true for GitHub Pages (HTTPS→WSS)
      debug: 1,              // log ICE/connection events to console for debugging
  ...
  async host() {
    ...
    this.peer = new Peer(code, this._peerOpts());
    ...
    return new Promise((resolve, reject) => {
      ...
      this.peer.on('error', err => {
        if (err.type === 'unavailable-id') {
          ...
          this.myId = code2;
          this.peer = new Peer(code2, this._peerOpts());
          this.peer.on('open', id => {
            this.myPeerId = id;
            this._showCode(id);
          ...
          this._status('Host error: ' + err.type, '#ff4400');
  ...
  _onGuestConnect(conn) {
    ...
    const onOpen = () => {
      ...
      conn.on('close', () => {
        ...
        if (this._remoteMeshes[pid]) {
          this.game.renderer?.scene?.remove(this._remoteMeshes[pid]);
      ...
      conn.on('error', e => {
        console.error('[NET guest conn error]', pid, e);
        this._status('Connection error with ' + pid, '#ff4400');
      });
    ...
    // conn may already be open (race condition) — handle both cases
    if (conn.open) {
      onOpen();
    } else {
      conn.on('open', onOpen);
  ...
  async join(code) {
    ...
    return new Promise((resolve, reject) => {
      // 30s timeout — ICE negotiation with TURN relay can take 10-20s
      ...
      this.peer.on('open', myId => {
        ...
        // NOTE: do NOT pass reliable:true — deprecated in PeerJS 1.5, breaks connections
        const conn = this.peer.connect(cleanCode, { serialization: 'json' });
        ...
        const onOpen = () => {
          ...
          conn.on('error', e => {
            console.error('[NET host conn error]', e);
            this._status('Connection error', '#ff4400');
          });

          // Send char select AFTER confirming the channel is open
          const myChar = CHARACTERS.find(c => c.id === (this.game.selectedChar || 'vex')) || CHARACTERS[0];
          conn.send({
            type: 'char_select',
            charId: myChar.id,
        ...
        // conn may already be open — handle both cases
        if (conn.open) {
          onOpen();
        } else {
          conn.on('open', onOpen);
        ...
        conn.on('error', e => {
          clearTimeout(timeout);
          console.error('[NET join conn error]', e);
          reject(e);
        });
      });
      ...
      this.peer.on('error', e => {
        clearTimeout(timeout);
        console.error('[NET peer error on join]', e);
        this._status('Error: ' + (e.type || e.message || e), '#ff4400');
        reject(e);
      });
    });
  }
