...
class NetManager {
  ...
  _onGuestConnect(conn) {
    ...
    conn.on('open', () => {
      ...
      // Send welcome with full lobby state so guest can sync immediately
      conn.send({
        type: 'lobby_welcome',
        yourPeerId: pid,
      ...
      conn.on('data', data => {
        if (!data) return;
        data._from = pid; // always stamp the true sender so routing is correct
        this._onMsg(data);
      });

      conn.on('error', e => console.warn('[NET] guest conn error:', pid, e));

      conn.on('close', () => {
        delete this.guestConns[pid];
        delete this.lobbySlots[pid];
        delete this.players[pid];
        delete this._remoteHealths[pid];
        delete this._playerVelocities[pid];
        delete this._playerLastUpdate[pid];
        ...
        if (this._remoteLabels[pid]) {
          try { this._remoteLabels[pid].remove(); } catch(_) {}
          delete this._remoteLabels[pid];
        }
  ...
  async join(code) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.peer.on('open', myId => {
        ...
        // 'open' fires once the data channel is ready
        const onOpen = () => {
  ...
  _onMsg(data) {
    ...
    switch (data.type) {
      ...
      case 'damage_event':
        // If this damage targets me, apply it
        if (data.targetId === this.myPeerId && this.game.player && this.game.player.isAlive) {
          ...
          if (!this.game.player.isAlive) {
            const killEvt = {
              ...
              killerTeam: (this.lobbySlots[data.killerId] || {}).team || 'b',
        ...
        // Host must relay damage to the target guest (if they're not the host)
        if (this.isHost) {
          if (data.targetId !== this.myPeerId) {
            this._sendToGuest(data.targetId, data);
          }
          // Also relay to all other guests so kill-feed stays in sync
      ...
      case 'kill_event': {
        ...
        const isVictim = data.victimId === this.myPeerId;
        if (this.game.hud) {
          ...
          if (isOurs) {
            ...
            if (this.game._addScore)      this.game._addScore(data.killerTeam || 'a', this.myPeerId);
            if (this.game._recordKill)    this.game._recordKill();
            if (this.game._spawnScorePopup) this.game._spawnScorePopup(100, 'KILL');
            if (this.game._screenShake)   this.game._screenShake(0.5, 120);
          } else if (!isVictim) {
            if (this.game._addScore) this.game._addScore(data.killerTeam || 'a', data.killerId);
          }
          if (isVictim) {
            if (this.game._playerDied) this.game._playerDied();
          }
        }
      ...
      case 'bot_kill_event':
        // A bot was killed by a player — update kill feed, score, and respawn the bot
        if (this.game.hud) {
          const isOurs = data.killerId === this.myPeerId;
          ...
          if (isOurs) {
            ...
            if (this.game.player) this.game.player.kills++;
            this.game._spawnScorePopup && this.game._spawnScorePopup(100, 'KILL');
          }
          this.game._addScore && this.game._addScore(data.killerTeam || 'a', data.killerId);
        }
        ...
        if (this.isHost && this.game.bots) {
          ...
          if (bot && bot.isAlive) {
            ...
            // Trigger respawn via game's _applyDamage path (just force health=0)
          }
          this._broadcast(data, fromId);
        }
        break;
  ...
  sendDamageEvent(targetPeerId, damage, killerName, weaponName) {
    ...
    if (this.isHost) {
      // If we (host) damaged ourselves, process locally
      if (targetPeerId === this.myPeerId) {
        this._onMsg({ ...evt });
      } else {
        // Send directly to the target guest
        this._sendToGuest(targetPeerId, evt);
        // Also broadcast to all others so kill-feed shows
        this._broadcast(evt, targetPeerId);
      }
    } else {
  ...
  // ── Heartbeat: detect zombie connections (open but silent) ────────────────
  _startHeartbeat() {
    this._stopHeartbeat();
      ...
      if (this.isHost) {
        for (const pid in this.guestConns) {
          ...
          if (!g.conn || !g.conn.open) {
            // Connection object says closed — clean up
            delete this.guestConns[pid];
            delete this.lobbySlots[pid];
            delete this.players[pid];
            this._broadcastLobby();
        ...
        this.isConnected = Object.keys(this.guestConns).length > 0;
      } else {
        if (this.hostConn && !this.hostConn.open) {
          this.isConnected = false;
          this._status('Connection to host lost', '#ff4400');
        }
      }
    }, 3000);
  }
