...
class NexusStrike {
  ...
  _buildUI(){
    ...
    this.on('back-from-lobby',()=>{
      this.net?.destroy();
      this.net=new NetManager(this);
      this.show('main-menu');this.state='menu';
    });
    ...
    for(let i=this.bullets.length-1;i>=0;i--){
      ...
      for(const t of targets){
        ...
        if(b.ownerId==='local'){
          ...
          if(t.isRemote && this.net?.isConnected){
            // Remote player target — send damage over network; kill handling via kill_event
            this.net.sendDamageEvent(t.peerId,dmg,this.net._getMyName(),this.player?.weaponSystem?.stats?.name||'WEAPON');
            // Don't add score or kill count here — the kill_event reply will do that
          } else {
            // Local target (bot or player)
            if(killed){
              this.player.kills++;
              this.hud.killNotif(t.name||'ENEMY');
  ...
  _quit(){
    ...
    document.exitPointerLock();
    this.net?.destroy();
    // Re-create NetManager so the next host/join works on a fresh peer
    this.net=new NetManager(this);
    this.player?.destroy();
